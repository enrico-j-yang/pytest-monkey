...
一个 pytest 插件和 CLI 工具，用于随机顺序多次执行测试，支持可复现的随机测试运行。类似安卓monkey工具。正常pytest运行测试是按顺序地全部用例运行一次的。这个工具是随机运行指定次数，比如10000次，遇到失败就停止（可配置失败不停止）。每次运行会打印随机种子值，需要复现/复测时指定种子值就可以按原来的顺序运行用例。
